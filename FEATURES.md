...
- **Plot preview / emulator** — Animate the real plot order on the viewport before running: server returns ordered polylines per pen (`/api/plot/preview-paths`), the client retimes from speed settings (play/pause, 1/5/20/100× speed, timeline scrubber). Travel moves show as faint dashed lines, pens draw in order in their own colours with pen-change markers on the timeline.
