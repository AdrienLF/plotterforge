...
- **Creative power and output quality:** 42 registered PFMs span Voronoi, LBG, adaptive, Poisson-disk, stipple, hatch, sketch, spiral, streamline, TSP, composite, mosaic, dither, and circle-packing families. Typed parameter schemas generate consistent controls. GPU acceleration uses CUDA or Metal/MPS with a CPU fallback. Layers support visibility, order, placement, crop, scale, occlusion, pens, and deterministic generators.
