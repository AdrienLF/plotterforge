...
- **Workflow and discoverability:** The three-step Compose → Generate → Plot structure is legible, primary actions are consistently disabled when invalid, image import creates a selected transformable raster layer, and the fast C12/M4 interaction counts show that the core path is not intrinsically cumbersome.
...
- **Creative power and output quality:** 50 registered PFMs span Voronoi, LBG, adaptive, Poisson-disk, stipple, hatch, sketch, spiral, streamline, TSP, composite, mosaic, tessellation, dither, shape-dither, circle-packing, and growth families. Typed parameter schemas generate consistent controls. GPU acceleration uses CUDA or Metal/MPS with a CPU fallback. Raster and vector layers support visibility, order, placement, crop, scale, rotation, fit/fill, occlusion, pens, and deterministic generators.
